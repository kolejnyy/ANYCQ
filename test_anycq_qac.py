...
def test_qac(config_file, model_dir, model_name, data_file, exp_name, _start=None, _end=None):
	...
	for i, cq_type in tqdm(enumerate(cq_types), total=end-start):
		...
		for pot_s in datum[1]['f1']:
			# Evaluate predictions for the positive answers
			with torch.no_grad():
				out_data = model(
						dataset.get_grounded(start + i, pot_s[0]),
						search_steps,
		...
		for pot_s in datum[2]['f1']:
			...
			print(torch.max(out_data.all_cq_scores.cpu(), dim=1)[0])

		print('fp = ', fp, '\t total = ', len(datum[2]['f1']))
...
if __name__ == '__main__':
	...
	parser.add_argument("--gen_type", type=str, default='')
	...
	data_name = f"qac_test_{args.gen_type}.json"

	test_qac(args.config_file, args.model_dir, args.model_name, data_name, exp_name, _start=args.start, _end=args.end)